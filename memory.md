...
更新时间：2026-08-19（`pick0.1_619`：抓取站直接 spawn、抬起后逆序回臂、后退 trim；walk 已能退后，原地右转待修）
...
**本阶段进展**：右手 Dex1 从 LoadingLine 托盘夹起 Product（v2 自然路点，已稳定）→ 胸口高度抬起 → 逆序关节回初始右臂 → `snap` 或 HOLD 后 `walk` 换站 → 灰筐上方松爪自由落下。夹取靠垫面摩擦，不焊 TCP。Walk 已能从抓取站后退到位；原地右转策略尚未迈步。

**归档状态（2026-08-19，`pick0.1_619`）**：

- 机器人 **spawn 就在抓取站**（`ROBOT_INIT_POS = PICK_STAND_XY`），启动不再瞬移。
- 抬起后新阶段 `RETURN_HOME`：按抓取路点 **逆序** 回到 00 / 默认右臂，夹爪保持闭合，回臂完成后再 walk。
- Walk 路线仍是「后退 → 右转 → 前进」固定幅值（§6）；`WALK_BACKOFF_TRIM=0.20` 让后退目标再收 20 cm（步子容易迈大）。
- Isaac Sim：后退能停到 backoff 点；**原地右转未执行**。规划器发的是满幅 `(0,0,-0.70)`，不是比例削命令。更像 settle 0.5 s 停稳后纯 `wz` 起不了步（§6.1 / §6.3）。
- Place 右臂轨迹（从初始臂姿抬臂再放、肘不外翻）**未完成**，本提交不改放置逻辑。
- 纯 CPU 导航测试已过。下一步只修 walk 右转（可加大 `WALK_WZ` 或右转时带一点平移把步态顶起来），不要动 pick/place。
...
## 6. Walk 换站（2026-08-18 重写，待 Isaac Sim 验收）
...
- 幅值在 `constants.py`：`WALK_VX=0.45`、`WALK_VY=0.30`、`WALK_WZ=0.70`。历史上 0.25–0.28 的 `vx` 只后仰不迈步（见 6.3），走不动就先加这里。
...
| 段 | kind | 内容 | 终点 |
|---|---|---|---|
| ① backoff | reverse | 负 `vx` 退出 CES，退到与灰色托盘中心对齐 | `(-2.267, -1.330)`（托盘中心 X −0.18 = 放置站 X） |
| ② turn_to_table | turn | 原地右转（`wz<0`），`yaw π → π/2` | 正对桌子 |
| ③ approach_place | forward | 正 `vx` 走世界 `+Y` 进放置站 | `PLACE_STAND_XY (-2.267, -0.772)` |

- 灰色托盘中心 = `PLACE_TARGET_XY (-2.087, -0.312)`（`place_gray_tray_on_table`：桌心 y−0.06）。
- 角点由 `build_carry_route` 求"后退线 × 放置站进入线"的交点算出，站位常量一改就自动跟着变。
- 进度按**行走轴投影**算（不是欧氏距离），侧向漂移不会让某段转不完。
- 每段之间零指令停稳 `WALK_LEG_SETTLE=0.5 s`，后退和右转不混在一条指令里。
- 平移中的纠偏也是固定幅值 + 带回差的死区：朝向差 > `WALK_ALIGN_YAW=0.30` 才发 `wz`，> `WALK_REALIGN_YAW=0.60` 就先停下转正；侧向差 > `WALK_LATERAL_TOL=0.10` 才发 `vy` 侧移（第③段靠它把 X 对齐误差吃掉）。
- 提前松油门：`WALK_STOP_MARGIN=0.15`，最后一段 `WALK_STOP_MARGIN_PLACE=0.12`（停短了手臂要多伸 → 够不到灰筐；停过头会撞桌）。**这两个是落点不准时的唯一旋钮。**
- 后退目标再收 `WALK_BACKOFF_TRIM=0.20`（实际比灰筐对齐线提前约 35 cm 松油门：trim + stop_margin）。
- 走路时不调用 `_apply_snap`、不移动 Product root；右臂保持 `_carry_arm_q`、Dex1 保持 `GRIPPER_CLOSED`，件仍只靠垫面摩擦。
- 到站稳定 `WALK_ARRIVE_HOLD=0.35 s` 后锁实际骨盆位姿，并打印与理想放置站的 `dxy/dyaw`。
- 走路到站有几厘米误差，`PLACE_HOLD_XY_M_WALK=0.05`：walk 模式的放置 IK **必须补 XY**（snap 模式仍用 0.40 只落 Z），否则件掉在灰筐外。

纯 CPU 测试：`python -m unittest tools.test_ces_walk_navigation`（10 项）。里面用"|vx|<0.30 / |wz|<0.40 一律不动"的死区模型仿真策略，验证路线仍能收敛、且**从不发小指令**。模型下落点误差 ≈ 6 cm、耗时 ≈ 7.3 s。

### 6.3 仍需在 Isaac Sim 验收的风险

全身 RL 策略按**默认垂臂站姿**训练。夹持后右臂前伸是 OOD：

- 负 `vx` 过小 → 只后仰不迈步然后倒下（这次靠 `WALK_VX=0.45` 解决，Isaac Sim 后退已迈步）
- **原地纯 `wz` 不转**：后退到位后 settle 0.5 s 再发 `(0,0,-0.70)`，人停住仍朝柜子。不是命令被比例削小；`WALK_WZ=0.70` 卡在键盘 Z/X 下限，斜坡前 0.2 s 还低于 `|wz|=0.40` 死区。
- 历史上 `vx` 0.25–0.28 仍弹飞 / 后仰踢腿
- 钉盆 + 策略蹬腿 → 物理爆炸
...
- [x] 路线不穿 CES：后退（世界 +X）离开机器后才转身
- [x] 后退段真的迈步，能停到 backoff 点（Isaac Sim 2026-08-19）
- [ ] 右转 90°：规划已发 `|wz|=0.70`，策略原地不起步（优先修这个）
- [ ] 到站 `dxy` 落在 ~10 cm 内，放置 IK 补 XY 后 TCP 在灰筐正上方
- [x] spawn 直接在抓取站，不再瞬移
- [ ] GUI 确认抓稳→桌边真实走路（右转还没过）
