...
关节状态作为 40 阶段起点，先回到 30，再经过 20 退出抽屉边缘，最后收到用户
重新设计的胸前 05：`40实时姿态 → 30 → 20 → 05`。三段原始时长为
`0.8 / 1.2 / 3.0` 秒，默认 `--ces_pick_speed 1.5` 下约为
`0.53 / 0.80 / 2.00` 秒。旧的 `ces_pick_natural_v1` 和
...
smoothstep 起止停稳。15 到位后读取真实 TCP，严格保留当时的 X/Y，只把 Z
交给现有笛卡尔插补器和位置 IK 向下补偿；不再追灰筐中心 X/Y，也不给朝向目标，
运行路径中没有 25。初始 Z 目标为灰筐上沿上方 `20 mm`；如果真实 15 已经更低，
“只降不升”策略不会反向抬手，也不会继续向下硬压。

经 20 回收和 Place `05→15` 合约已经覆盖 CPU 测试；产品避碰、最终 Z 余量、
夹持摩擦、释放和持物行走平衡仍需在 Isaac Sim 中验收。
