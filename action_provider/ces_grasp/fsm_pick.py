...
class CesPickMixin:
    ...
    def _start_grasp(self, err: float):
        ...
        _target, yaw_err = closer_world_x_yaw(yaw_end)
        print(
    ...
    def _begin_descend(self):
        ...
        self._log_wp("live_30", self.ctx.get_right_arm_q()[0], self._q_wp30)
    ...
    def _start_joint_waypoints(self):
        ...
        if lead > C.WAYPOINT_LEAD_IN_TOL:
            ...
            self._wp_arrive_names = list(names)
            print(
            ...
            qs[0] = q_now
            self._wp_arrive_names = list(names[1:])
            self._log_wp(names[0], q_now)
            print(
    ...
    def _flush_wp_arrivals(self, q: torch.Tensor):
        while (
            ...
            and self.joint_interp.elapsed + 1e-9
            >= self.joint_interp.bounds[self._wp_logged]
        ):
            name = (
            ...
            if self._wp_logged > 0:
                ...
                if prev_name in self._trajectory.q_by_name:
    ...
    def _q_limit_margin(self, q: torch.Tensor) -> float:
        ...
        if ik is None or not hasattr(ik, "q_min"):
            return float("nan")
        qv = q.reshape(-1)[:7]
        return float(torch.minimum(qv - ik.q_min, ik.q_max - qv).min().item())

    def _tcp_in_pelvis(self):
        root_pos, root_quat = self.ctx.get_base_pose_w()
        tcp_pos, tcp_quat = self.ctx.ik.get_tcp_pose_w()
        pos_b, quat_b = subtract_frame_transforms(
            root_pos, root_quat, tcp_pos, tcp_quat
        )
        return pos_b[0], quat_b[0]

    def _log_wp(
        self, name: str, q: torch.Tensor, prev_q: torch.Tensor | None = None
    ):
        delta = ""
        if prev_q is not None:
            dmax = float(
                (q.reshape(-1)[:7] - prev_q.reshape(-1)[:7]).abs().max().item()
            )
            delta = f" dmax={dmax:.3f}rad"
        print(
            f"[ces_verify] wp={name} q={self._fmt_q(q)} "
            f"limit_margin={self._q_limit_margin(q):.3f}rad{delta}"
        )
    ...
    def _log_tcp(self, tag: str):
        ...
        if "place" in tag:
            ...
            return pb
        target = (0.320, -0.380, 0.181) if "30" in tag or "handoff" in tag else (
            0.320,
            -0.380,
            0.101,
        )
        err = tuple(pb[i] - target[i] for i in range(3))
        print(
            f"[ces_verify] {tag} tcp_w=({pw[0]:.4f},{pw[1]:.4f},{pw[2]:.4f}) "
            f"tcp_b=({pb[0]:.4f},{pb[1]:.4f},{pb[2]:.4f}) "
            f"vs({target[0]:.3f},{target[1]:.3f},{target[2]:.3f}) "
    ...
    def _step_descend(self):
        ...
        if q_ref is not None and not self._logged_q_ref_once:
            self._logged_q_ref_once = True
            live = self.ctx.get_right_arm_q()[0]
            wrist = ""
            if self._q_wp30 is not None:
                wrist = f" wrist_vs_30={self._fmt_q((live-self._q_wp30)[4:7])}"
            print(
                f"[ces_verify] descend q_ref_only (arm_q is None) "
                f"q_ref={self._fmt_q(q_ref)}{wrist}"
            )
        descend_limit = (
